...
class MelodyTokenizer:
    ...
    def get_notes(self, pitches: List[int], starts: List[float], ends: List[float]) -> List[Note]:
        iois = []
        durations = []
        ioi_ratios = []
...
class FantasticTokenizer(MelodyTokenizer):
    ...
    def tokenize_melody(self, pitches: List[int], starts: List[float], ends: List[float]) -> List[List]:
    ...
    def segment_melody(self, notes: List[Note]) -> List[List]:
        ...
        for note in notes:
            ...
            need_new_phrase = len(current_phrase) > 0 and note.start - current_phrase[-1].end > self.phrase_gap
            if need_new_phrase:
    ...
    def count_grams(
            ...
            n: int,
            existing: Optional[Counter] = None
        ) -> Counter:
        ...
        for i in range(len(sequence) - n + 1):
            ...
            ngram = tuple(tuple(x) if isinstance(x, list) else x for x in sequence[i:i+n])
            existing[ngram] += 1
