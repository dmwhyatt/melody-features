...
class ExperimentLogger:
    ...
    def generate_experiment_history_folder(self):
        if self.experiment_history_folder_path is None:
            ...
            if not experiment_history_folder.endswith('/'):
                experiment_history_folder += '/'
    ...
    def _get_files_from_paths(self, path):  # only two types files allowed: midi and kern
        files = []
        # Ensure path ends with a slash for proper glob pattern
        if not path.endswith('/'):
            path = path + '/'
        for file in glob(path + '*'):
            if file.endswith(".mid") or file.endswith(".krn"):
                files.append(file)
